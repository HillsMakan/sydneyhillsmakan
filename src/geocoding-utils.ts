...
export function simplifyAddress(address: string): string[] {
  ...
  let current = address.replace(/\([^)]*\)/g, '').replace(/\s+/g, ' ').trim();
  current = current.replace(/\s*,\s*/g, ', ').replace(/[\\,]+$/, '').trim();
  results.push(current);

  // 2. Remove business prefixes (Shop, Unit, Level, etc.)
  let noPrefix = current.replace(/^(Shop|Unit|Level|Suite|SShop|Pork|Ribs)\s+[^,]+,\s*/i, '');
  if (noPrefix !== current) {
...
export async function geocode(query: string, regionId: string, strict = false) {
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      if (strict) {
          const isGeneric = ['country', 'state', 'province', 'region', 'administrative'].includes(bestMatch.type) || 
